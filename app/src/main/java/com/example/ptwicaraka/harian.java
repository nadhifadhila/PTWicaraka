        ...
        submitharian.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                reference = FirebaseDatabase.getInstance().getReference()
                        .child("laporan_harian").child(username_key_new).child(id.toString());
                reference.addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        CharSequence select = mDisplayDate.getText();
                        String selected = spinner1.getSelectedItem().toString();
                        dataSnapshot.getRef().child("id_harian").setValue(id.toString());
                        dataSnapshot.getRef().child("tanggal").setValue(select);
                        dataSnapshot.getRef().child("nama_pembeli").setValue(edt_nama.getText().toString());
                        dataSnapshot.getRef().child("alamat_pembeli").setValue(edt_alamat.getText().toString());
                        dataSnapshot.getRef().child("kategori").setValue(selected);
                        dataSnapshot.getRef().child("keterangan").setValue(edt_ket.getText().toString());
                        dataSnapshot.getRef().child("jumlah_pembelian tabung").setValue(edt_jmlbeli.getText().toString());
